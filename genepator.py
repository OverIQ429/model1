...
def create_like_async(session, post_id, user_id, foi, publication_date):
    like_data = {
        "getfio":  foi,
        #"publicationDate": publication_date,
        "postId":  post_id,
    ...
    with session.post(f"{BASE_URL}/likes/", json=like_data) as response:
...
def create_likes_async(posts, users):
    with aiohttp.ClientSession() as session:
        ...
        for post in posts:
            ...
            for _ in range(num_likes):
                ...
                like = asyncio.create_task(create_like_async(session, post["identifier"], user["identifier"], user["fio"], like_date)) #Передаем fio
                likes.append(like)
...
def main():
    ...
    if args.users:
        ...
        if args.posts and users:
            ...
            if posts:
                print("Создаем лайки...")
                asyncio.run(create_likes_async(posts, users))
